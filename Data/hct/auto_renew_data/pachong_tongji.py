...
def update_interaction_detail(conn, df):  
    with conn.cursor() as cursor:  
        cursor.execute("DELETE FROM \"同济医院交互明细\";")
        for index, row in df.iterrows():  
            cursor.execute("""  
                INSERT INTO "同济医院交互明细" (医院名称, 交互类型, 时间, 链接)   
                VALUES (%s, %s, %s, %s)  
...
def update_interaction_count(conn, df):  
    with conn.cursor() as cursor:
        # 删除表中的所有数据
        cursor.execute("DELETE FROM \"同济医院交互强度表\";")
        # 插入新数据
        for index, row in df.iterrows():
            cursor.execute("""
                INSERT INTO "同济医院交互强度表" (医院名称, 文章数量, 合作, 沟通, 技术)
                VALUES (%s, %s, %s, %s, %s)
...
if __name__ == "__main__":
    # 调用函数并传入文件路径
    file_path = r"同济.txt"
    output_detail = r"同济医院交互明细.xlsx"
    output_count = r"同济医院交互强度表.xlsx"
