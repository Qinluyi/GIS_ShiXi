...
def count_hospitals_from_file(file_path,output_detail, output_count):
    ...
    try:
        ...
        if conn:
            ...
            update_interaction_count(conn,df1)
            conn.close()
            print("武汉大学中南医院数据已成功更新到数据库中。")
...
def update_interaction_detail(conn, df):  
    with conn.cursor() as cursor:  
        cursor.execute("DELETE FROM \"武汉大学中南医院交互明细\";")
        for index, row in df.iterrows():  
            cursor.execute("""  
                INSERT INTO "武汉大学中南医院交互明细" (医院名称, 交互类型, 时间, 链接)   
                VALUES (%s, %s, %s, %s)  
...
def update_interaction_count(conn, df):  
    with conn.cursor() as cursor:  
        cursor.execute("DELETE FROM \"武汉大学中南医院交互强度表\";")
        for index, row in df.iterrows():  
            cursor.execute("""  
                INSERT INTO "武汉大学中南医院交互强度表" (医院名称, 文章数量, 合作, 沟通, 技术)   
                VALUES (%s, %s, %s, %s, %s)  
...
if __name__ == "__main__":
    # 调用函数并传入文件路径
    file_path = r"中南.txt"
    output_detail = r"武汉大学中南医院交互明细.xlsx"
    output_count = r"武汉大学中南医院交互强度表.xlsx"
