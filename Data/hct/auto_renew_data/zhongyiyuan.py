...
def update_interaction_detail(conn, df):  
    with conn.cursor() as cursor:  
        cursor.execute("DELETE FROM \"湖北省中医院交互明细\";")
        for index, row in df.iterrows():  
            cursor.execute("""  
                INSERT INTO "湖北省中医院交互明细" (医院名称, 交互类型, 时间, 链接)   
                VALUES (%s, %s, %s, %s)  
...
def update_interaction_count(conn, df):  
    with conn.cursor() as cursor:  
        cursor.execute("DELETE FROM \"湖北省中医院交互强度表\";")
        for index, row in df.iterrows():  
            cursor.execute("""  
                INSERT INTO "湖北省中医院交互强度表" (医院名称, 文章数量, 合作, 沟通, 技术)   
                VALUES (%s, %s, %s, %s, %s)  
