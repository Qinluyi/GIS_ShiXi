...
def count_hospitals_from_file(file_path,output_excel_path):
    try:
        ...
        conn=connect_db()
        if conn:
            update_interaction_count(conn,df)
            conn.close()
        print("武汉大学口腔医院数据已成功更新到数据库中1。")
...
def update_interaction_count(conn, df):  
    with conn.cursor() as cursor:  
        cursor.execute("DELETE FROM \"武汉大学口腔医院交互强度表\";")
        for index, row in df.iterrows():  
            cursor.execute("""  
                INSERT INTO "武汉大学口腔医院交互强度表" (医院名称, 文章数量, 合作, 沟通, 技术)   
                VALUES (%s, %s, %s, %s, %s)  
...
if __name__ == "__main__":
    ...
    output_excel_path = r"武汉大学口腔医院交互强度表.xlsx"
